      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            ...
            TPGHRC005db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              TPGHRC005db['GAPnameList'].push(graph[k]['NO']);
            }
          }
          ...
          TPGHRC005db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
...
router.post('/TPGHRC005-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--TPGHRC005-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
...
router.post('/TPGHRC005-feedback', async (req, res) => {
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      if (TPGHRC005db['RESULTFORMAT'] === 'Number' || TPGHRC005db['RESULTFORMAT'] === 'Text' || TPGHRC005db['RESULTFORMAT'] === 'Graph') {
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (TPGHRC005db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (TPGHRC005db['INTERSEC'] !== '') {
                core = parseFloat(TPGHRC005db['INTERSEC'])
              } else {
...
router.post('/TPGHRC005-FINISH', async (req, res) => {
  ...
  if (TPGHRC005db['RESULTFORMAT'] === 'Number' || TPGHRC005db['RESULTFORMAT'] === 'Text') {
    ...
    for (i = 0; i < TPGHRC005db['confirmdata'].length; i++) {
      TPGHRC005db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": TPGHRC005db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (TPGHRC005db['RESULTFORMAT'] === 'Number' ||
    ...
    TPGHRC005db['RESULTFORMAT'] === 'Picture' || TPGHRC005db['RESULTFORMAT'] === 'Graph') {
    request.post(
