...
router.post('/GRAPH-recal-New', async (req, res) => {
  //-------------------------------------
  console.log('--GRAPH-recal-New--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined  ) {
      // console.log(Object.keys(feedback[0]['FINAL'][NAME_INS][input["ITEMs"]]));
      // 
      let obs = ''
      if(feedback[0]['FINAL']["TPG-HMV-002"] != undefined){
         obs = "TPG-HMV-002";
      }else if(feedback[0]['FINAL']["TPG-HMV-003"] != undefined){
        obs = "TPG-HMV-003";
      }else{
        return res.json(output);
      }
      let ob = feedback[0]['FINAL'][obs][input["ITEMs"]];

      let oblist = Object.keys(feedback[0]['FINAL'][obs][input["ITEMs"]]);
...
router.post('/GASGWreport', async (req, res) => {
  ...
  output = await mongodb.findproject(MAIN_DATA, MAIN, out,{"PO":1,"CP":1,"MATCP":1,"CUSTOMER":1,"PART":1,"PARTNAME":1,"MATERIAL":1,"CUSLOTNO":1, "IDInspected": 1 , "IDCheck": 1 , "IDApprove": 1 , "IDApprove": 1 , "FG_CHARG": 1 });
  console.log(output)
