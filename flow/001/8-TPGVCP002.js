...
router.get('/CHECK-TPGVCP002', async (req, res) => {

  return res.json(TPGVCP002db['PO']);
});


router.post('/TPGVCP002db', async (req, res) => {
  //-------------------------------------
  // console.log('--TPGVCP002db--');
...
router.post('/GETINtoTPGVCP002', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoTPGVCP002--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGVCP002-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < TPGVCP002db['ItemPickcode'].length; i++) {
    if (TPGVCP002db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = TPGVCP002db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    TPGVCP002db['inspectionItem'] = ITEMSS;
    TPGVCP002db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': TPGVCP002db["PO"], 'CP': TPGVCP002db["CP"], 'ITEMs': TPGVCP002db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": TPGVCP002db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            TPGVCP002db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            TPGVCP002db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            TPGVCP002db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              TPGVCP002db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              TPGVCP002db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          TPGVCP002db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          TPGVCP002db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          TPGVCP002db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          TPGVCP002db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16000/TPGVCP002-feedback',
              { json: { "PO": TPGVCP002db['PO'], "ITEMs": TPGVCP002db['inspectionItem'] } },
...
router.post('/TPGVCP002-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    TPGVCP002db['GAPnameListdata'] = graph[0];//confirmdata
    TPGVCP002db['GAP'] = TPGVCP002db['GAPnameListdata'][`GT${TPGVCP002db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/TPGVCP002-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--TPGVCP002-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGVCP002-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = TPGVCP002db['preview'][0]

    if (TPGVCP002db['RESULTFORMAT'] === 'Graph') {
      let pushdata = TPGVCP002db['preview'][0]

      pushdata['V5'] = TPGVCP002db['GAP'];
      pushdata['V1'] = `${TPGVCP002db['confirmdata'].length + 1}:${pushdata['V1']}`;

      TPGVCP002db['confirmdata'].push(pushdata);
      TPGVCP002db['preview'] = [];
      output = 'OK';
      TPGVCP002db['GAP'] = TPGVCP002db['GAPnameListdata'][`GT${TPGVCP002db['confirmdata'].length + 1}`]

    } else if (TPGVCP002db['RESULTFORMAT'] === 'Number') {

      let pushdata = TPGVCP002db['preview'][0]

      pushdata['V5'] = TPGVCP002db['confirmdata'].length + 1
      pushdata['V1'] = `${TPGVCP002db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/TPGVCP002-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      TPGVCP002db["PCSleft"] = `${parseInt(TPGVCP002db["PCS"]) - oblist.length}`;
      if (TPGVCP002db['RESULTFORMAT'] === 'Number' || TPGVCP002db['RESULTFORMAT'] === 'Text' || TPGVCP002db['RESULTFORMAT'] === 'Graph') {
        ...
        TPGVCP002db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        TPGVCP002db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(TPGVCP002db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (TPGVCP002db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (TPGVCP002db['INTERSEC'] !== '') {
                core = parseFloat(TPGVCP002db['INTERSEC'])
              } else {
              ...
              try {
                ...
                TPGVCP002db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (TPGVCP002db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
      ...
      TPGVCP002db["ITEMleftUNIT"] = '';
      TPGVCP002db["ITEMleftVALUE"] = '';
    }
...
router.post('/TPGVCP002-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGVCP002-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/TPGVCP002-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = TPGVCP002db['confirmdata'].length
    if (all > 0) {
      TPGVCP002db['confirmdata'].pop();
    }
...
router.post('/TPGVCP002-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--TPGVCP002-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (TPGVCP002db['RESULTFORMAT'] === 'Number' || TPGVCP002db['RESULTFORMAT'] === 'Text') {

    TPGVCP002db["value"] = [];
    console.log(TPGVCP002db["value"]);
    for (i = 0; i < TPGVCP002db['confirmdata'].length; i++) {
      TPGVCP002db["value"].push({
        "PO1": TPGVCP002db["inspectionItemNAME"],
        "PO2": TPGVCP002db['confirmdata'][i]['V1'],
        "PO3": TPGVCP002db['confirmdata'][i]['V2'],
        "PO4": TPGVCP002db['confirmdata'][i]['V3'],
        "PO5": TPGVCP002db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (TPGVCP002db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < TPGVCP002db["value"].length; i++) {
        mean01.push(parseFloat(TPGVCP002db["value"][i]["PO3"]));
        mean02.push(parseFloat(TPGVCP002db["value"][i]["PO5"]));
      }
      ...
      TPGVCP002db["value"].push({
        "PO1": 'Mean',
        "PO2": TPGVCP002db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": TPGVCP002db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (TPGVCP002db['RESULTFORMAT'] === 'OCR' || TPGVCP002db['RESULTFORMAT'] === 'Picture') {

  } else if (TPGVCP002db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < TPGVCP002db['confirmdata'].length; i++) {
      TPGVCP002db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": TPGVCP002db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (TPGVCP002db['RESULTFORMAT'] === 'Number' ||
    TPGVCP002db['RESULTFORMAT'] === 'Text' ||
    TPGVCP002db['RESULTFORMAT'] === 'OCR' ||
    TPGVCP002db['RESULTFORMAT'] === 'Picture' || TPGVCP002db['RESULTFORMAT'] === 'Graph') {
    request.post(
